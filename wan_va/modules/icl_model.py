...
class ICLAttentionBackend:
    ...
    def _build_mask(mask_mod, query_length, key_length, device, compile_mask):
        ...
        if device.type != "cuda":
            ...
            # Keep the final boolean mask, but bound temporary integer matrices.
            # The predicates otherwise allocate several full Q x K matrices
            # (including int64 intermediates on Ascend) before producing bools.
            if query_length * key_length <= _DENSE_MASK_DIRECT_ELEMENTS:
                q_idx = torch.arange(query_length, device=device)[:, None]
                mask = mask_mod(index, index, q_idx, kv_idx)[None, None]
                if device.type == "npu":
                    mask._valid_query_rows = mask.any(dim=-1, keepdim=True)
                return mask
            rows = max(1, min(query_length, _DENSE_MASK_TILE_ELEMENTS // max(key_length, 1)))
            mask = torch.empty((query_length, key_length), device=device,
                               dtype=torch.bool)
    ...
    def apply(
        ...
        if isinstance(block_mask, torch.Tensor):
            ...
            if query.device.type == "npu":
                # NPU SDPA can return nonzero values for all-masked rows.
                # Match SDPA's zero output/gradient semantics for padding.
                # Cache the row reduction on our immutable per-forward masks.
                valid_rows = getattr(block_mask, '_valid_query_rows', None)
                if valid_rows is None:
                    valid_rows = block_mask.any(dim=-1, keepdim=True)
...
class WanICLTransformer3DModel(ModelMixin, ConfigMixin):
    ...
    def _forward_training_mcp(
        ...
        for module_index, (group, stream) in enumerate(
            ...
            # All MCP depths use the same visibility metadata. Future offsets
            # change RoPE/noising, not these mask predicates. Share immutable
            # masks within this forward (also retained for AC recomputation).
            if module_index == 0:
